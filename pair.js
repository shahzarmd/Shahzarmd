...
router.get('/', async (req, res) => {
    ...
    async function createSocketSession() {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                try {
                    ...
                    const successMsg = {
                        text:
                            `🚀 *SHAHZAR-MD Session Created!*\n\n` +
                            `▸ *Never share* your session ID\n` +
                            `▸ Join our WhatsApp Channel\n` +
                            `▸ Report bugs on GitHub\n\n` +
                            `_Powered by SHAHZAR-MD\n\n` +
                            `🔗 *Useful Links:*\n` +
                            `▸ GitHub: https://github.com/shahzarmd/shahzarmd\n` +
                            `▸ https://whatsapp.com/channel/0029VbAeu4REVccNAW2zQb0N`,
                        contextInfo: {
        ...
        if (!sock.authState.creds.registered) {
            await delay(1500);
            const pairingCode = await sock.requestPairingCode(phoneNumber, "SHAHZAR1");
            if (!res.headersSent) {
